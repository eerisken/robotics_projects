...
class ProbabilisticOccupancyMap:
    # Mapping parameters
    ...
    def update_map(self, robot_x, robot_y, robot_heading, lidar_range_image, lidar_horizontal_resolution, lidar_field_of_view, lidar_max_range):
        ...
        lidar_hits_r = np.array([ranges*np.cos(angles), ranges*np.sin(angles), np.ones(ranges.shape[0])])
        lidar_hits_world = w_T_r @ lidar_hits_r
        
        for k in range(lidar_hits_world.shape[1]):
    ...
    def grow_obstacles(self):
        ...
        convolved_map = convolve(binary_obstacles, kernel, mode='constant', cval=0.0)
